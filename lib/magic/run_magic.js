...
module.exports.run = function(file, global_ns, callback) {
  ...
  if (ext == '.js') {
    js_run(content, filename, global_ns, callback, this.repl_server.transform);
  }
  else if (ext == '.html') {
...
var js_run = function(content, filename, global_ns, callback, transform) {
  ...
  if (lines[0].substr(0, 7) == '// %run') {
    ...
    return module.exports.run(file, global_ns, function() {
      if (!only_html) {
        return js_run(content, filename, global_ns, null, transform);
      }
    });
  }
  ...
  sandbox['console'] = global_ns.console;
  ...
  // init file stuff so its require is based on the dirname
  // Normamly this is done via load, which we don't want
  // because we are running the code in Contextify and don't 
  // want it to error out in Module._compile
  new_mod.paths = Module._nodeModulePaths(dirname);
  new_mod.filename = filename;
  ...
  sandbox.exports = new_mod.exports;
  ...
  try {
    sandbox.run(content, filename);
  }
  finally {
    // TODO: Should this be more restrictive?
    let sglobal = sandbox.getGlobal();
    for (var k in sglobal) {
      global_ns[k] = sglobal[k];
    }
  }
  // oooh fun. if running something async this will segfault me
  // hmmmm
  //sandbox.dispose();
...
function html_run(content, filename, global_ns, callback) {
  ...
  load_script.innerHTML = "_choir_sentinel = '_choir_sentinel';";
  ...
  global_ns['_http_callback'] = function() {
    return html.stripScripts(document.documentElement.innerHTML);
  }
  if (callback) {
    callback();
  }
