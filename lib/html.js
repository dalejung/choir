...
module.exports.create_dom = function (url) {
   ...
   */
  var document = jsdom.jsdom("<html><head></head><body></body></html>", {url});
...
module.exports.window_vars = function (window, default_keys, deferred) {
  // checks for the choir_sentinel value added to each html run
  if (!('_choir_sentinel' in window)) {
    setTimeout(module.exports.window_vars, 100, window, default_keys, deferred);
  } else {
    var choir_deferred = window._choir_load;
    if (choir_deferred) {
      if(choir_deferred.promise) {
        choir_deferred = choir_deferred.promise;
      }
      choir_deferred.timeout(5000, 'Page load timeout')
        .then(function() {
          deferred.resolve();})
        .fail(function() {
...
module.exports.stripScripts = function(html) {
  ...
  while (i--) {
    ...
    if (!(script.getAttribute('choir-keep') == 'true')) {
