...
var browserjs = require('./browserjs.js');
...
var http_handler = function(request, res) {
  ...
  if (uri == '/') {
    return choir_base_handler(request, res, repl_kernels)
  }

  if (uri == '/commonjs') {
    return browserjs.commonjs(res);
  }

  if (uri.indexOf('/require/') == 0) {
    return browserjs.requirejs(uri, res);
  }
  ...
  browserResolve(uri.slice(1), opts, function(err, path, info) {
    if (path) { // found browser path
      filename = path;
      name = info['name'];
      return browserjs.browserify_od(filename, res, name);
    }
...
function static_js_handler(filename, file, res) {
  if (is_common_js(file)) {
    return browserjs.browserify_od(filename, res);
  }
...
function choir_base_handler(req, res, repl_kernels) {
